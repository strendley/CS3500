...
# hw2_sample_input writing the output to a directory named
# hw2_actual_output
#
# Finally, it will do a diff on each file in hw2_actual_output
# against the file with the same name that is in a directory
# named hw2_expected_output and report the differences in a
# directory named reports3
#
# To run this script, do the following:
# (1) Make sure you do NOT have a directory named hw2_actual_output
# (2) Make sure you do NOT have a directory named reports3
# (3) Make sure you have a directory named hw2_sample_input that
#     contains ONLY the sample input files you want to test
# (4) Make sure you have a directory named hw2_expected_output
#     that contains ONLY the expected output files, one for each
#     of the files in the hw2_sample_input directory
...
# while running the script, rm the hw2_actual_output and reports3
...
inputs=`ls HW2_sample_input --ignore-backups`
echo "--> Running against test cases"
for i in $inputs; do
  echo "--> Trying" $i
  ./a.out < ./HW2_sample_input/$i > ./HW2_actual_output/$i.out

  diff ./HW2_actual_output/$i.out ./HW2_expected_output/$i.out > \
       ./reports3/$i --ignore-space-change --ignore-case \
       --side-by-side

  lines=`diff ./HW2_actual_output/$i.out ./HW2_expected_output/$i.out \
         --ignore-space-change --ignore-case --ignore-blank-lines  | wc --lines`
