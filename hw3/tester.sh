...
inputs=`ls HW3_sample_input --ignore-backups`
echo "--> Running against test cases"
for i in $inputs; do
  echo "--> Trying" $i
  ./a.out < ./HW3_sample_input/$i > ./HW3_actual_output/$i.out

  diff ./HW3_actual_output/$i.out ./HW3_expected_output/$i.out > \
       ./reports3/$i --ignore-space-change --ignore-case \
       --side-by-side

  lines=`diff ./HW3_actual_output/$i.out ./HW3_expected_output/$i.out \
